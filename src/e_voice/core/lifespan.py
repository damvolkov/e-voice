"""Lifespan management with event-based architecture for e-voice."""
...
class Lifespan:
    ...
    def startup(self) -> AsyncHandler:
        ...
        async def _startup() -> None:
            logger.info("starting application lifespan", version=st.API_VERSION, step="START")
            self._state = State()
            ...
            for event_cls in self._event_classes:
                ...
                logger.info(f"starting event: {event.name}", step="START")
                instance = await event.startup()
                setattr(self._state, event.name, instance)
                logger.info(f"event ready: {event.name}", step="OK")

                self._events.append(event)

            self._app.inject_global(state=self._state)
            logger.info("app state ready", step="OK")
    ...
    def shutdown(self) -> AsyncHandler:
        ...
        async def _shutdown() -> None:
            logger.info("cleaning up app state", step="STOP")

            if not self._state:
                logger.warning("no state to cleanup")
                return

            for event in reversed(self._events):
                if event.has_shutdown() and event.name in self._state:
                    logger.info(f"shutting down: {event.name}", step="STOP")
                    instance = getattr(self._state, event.name)
                    await event.shutdown(instance)
                    logger.info(f"shutdown complete: {event.name}", step="OK")

            self._state.clear()
            logger.info("cleanup complete", step="OK")
